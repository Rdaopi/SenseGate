...
static void add_round_key(uint8_t state[4][4], int round)
{
    for (int r = 0; r < 4; r++)
        for (int c = 0; c < 4; c++)
            state[r][c] ^= round_key[round*16 + c*4 + r];
}
...
int aes_ctr_init(const uint8_t *key, size_t key_len)
{
    if (key_len != AES_KEY_SIZE) {
        printk("AES: invalid key length %zu\n", key_len);
        return -1;
    }
    key_expansion(key);
    initialized = 1;
    printk("AES-128 (pure C) initialized OK\n");
    return 0;
}
                   ...
                   size_t         len)
{
    ...
    size_t offset = 0;
    while (offset < len) {
        aes_encrypt_block(counter, keystream);
